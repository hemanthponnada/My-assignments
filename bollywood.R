...
# Store the movie name in variable named top_movie
  ...
  top_movie <-bollywood$Movie[bollywood$Tcollection==max(bollywood$Tcollection)]
...
# Store the movie name in variable named top_2_movie
  T<-sort(bollywood$Tcollection,decreasing=TRUE)
  top_2_movie <- bollywood$Movie[bollywood$Tcollection==T[2]]
...
# Write command/s to find out how many movies are in Flop, Average, Hit and Superhit categories in the entire Bollywood data set.
	...
	#Number of movies in flop 
	length(bollywood$Movie[bollywood$Verdict=="Flop"])
	#Number of movies in Average
	length(bollywood$Movie[bollywood$Verdict=="Average"])
	#Number of movies in Hit
	length(bollywood$Movie[bollywood$Verdict=="Hit"])
	#Number of movies in Super Hit
	length(bollywood$Movie[bollywood$Verdict=="Super Hit"])
...
# Store the names of 4 movies in same sequence in movie_result vector
  ...
  movie_result <-bollywood$Movie[c(which.max(bollywood$Ocollection),which.max(bollywood$Wcollection),which.max(bollywood$Fwcollection),which.max(bollywood$Tcollection))]
